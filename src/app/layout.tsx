      ...
      <body>
        ...
        <div className="flex min-h-screen">
          {/* Nav liest ?view= via useSearchParams → Suspense-Grenze, damit statisches Prerendering (z. B. /_not-found) funktioniert.
              Fallback reserviert exakt die Sidebar-Breite, damit es keinen Layout-Sprung gibt. */}
          <Suspense fallback={<div aria-hidden className="sticky top-0 hidden h-screen w-[256px] shrink-0 border-r border-[var(--color-line)] bg-[var(--color-surface)] sm:block" />}>
            <Nav />
          </Suspense>
          <main className="min-w-0 flex-1 px-5 py-7 sm:px-8 lg:px-10 xl:px-12">{children}</main>
        </div>
        <Toaster />
      </body>
    </html>
